...
//     if (_userInitiatedDisconnect) return;zzzzzz
...
class BleProvider extends ChangeNotifier {
  ...
  void startScan() {
    ...
    _bleScanSubscription = _ble
        ...
        .listen(
          (device) {
            log(
              'Found: ${device.name} | ${device.id} | connectable=${device.connectable}',
            );
  ...
  // Connect to Ble Device
  Future<void> connectToBleDevice(DiscoveredDevice device) async {
    if (_bleConnectedDevice?.id == device.id && isBleConnected) return;
